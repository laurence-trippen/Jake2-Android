...
public final class FS extends Globals {
    ...
    public static void InitFilesystem() {
        ...
        //
        File file = new File("./baseq2");
        System.out.println("LT: " + file.getAbsolutePath());
        if (file.exists()) {
            fs_basedir = Cvar.getInstance().Get("basedir", ".", CVAR_NOSET);

            System.out.println("LT 1: " + fs_basedir);
        } else {
            String autoDetectedBasedir = autodetectBasedir();
            fs_basedir = Cvar.getInstance().Get("basedir", autoDetectedBasedir, CVAR_NOSET);

            System.out.println("LT 2: " + fs_basedir);
        }
    ...
    // TODO LT: Fix auto-detect for android? Maybe on internal or external SD location
